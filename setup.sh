...
#------------------------------------------------------------------------------
initial_config() {
    # If in FULL_INSTALL mode, ask for credentials at the beginning
    if $FULL_INSTALL; then
    ...
    clear
    show_banner
    execute_command "pkg update -y -o Dpkg::Options::=\"--force-confold\"" "Updating repositories"
    execute_command "pkg upgrade -y -o Dpkg::Options::=\"--force-confold\"" "Upgrading packages"
    ...
    if $SHELL_CHOICE; then
        ...
        case $SHELL_CHOICE in
            ...
            "zsh")
                ...
                # Install Oh My Zsh and other ZSH configurations
                ...
                execute_command "curl -fLo \"$ZSHRC\" https://raw.githubusercontent.com/GiGiDKR/OhMyTermux/dev/src/zshrc" "Default configuration" || error_msg "Failed to apply default configuration"

                if command -v zsh &> /dev/null; then
                    install_zsh_plugins
                    install_prompt
                else
                    echo -e "${COLOR_RED}ZSH is not installed. Cannot install plugins.${COLOR_RESET}"
                fi
                chsh -s zsh
                ;;
            "fish")
                ...
                execute_command "mkdir -p $HOME/.config/fish/functions" "Creating fish directory"
                # Install Fisher in non-interactive mode
                execute_command "curl -sL https://raw.githubusercontent.com/jorgebucaran/fisher/main/functions/fisher.fish -o $HOME/.config/fish/functions/fisher.fish" "Downloading Fisher"
                # Install Tide via Fisher in non-interactive mode
                execute_command "fish -c 'source $HOME/.config/fish/functions/fisher.fish && fisher install IlanCosman/tide@v5'" "Installing Tide"
                chsh -s fish
                ;;
        esac
    fi
...
install_prompt() {
    ...
    if [ "$CURRENT_SHELL" = "bash" ]; then
        if $USE_GUM; then
            PROMPT_CHOICE=$(gum_choose "Choose prompt to install:" --height=4 --selected="Oh-My-Posh" "Oh-My-Posh" "Starship")
        else
            echo -e "${COLOR_BLUE}Choose prompt to install:${COLOR_RESET}"
            echo
        ...
        if $USE_GUM; then
            PROMPT_CHOICE=$(gum_choose "Choose prompt to install:" --height=5 --selected="PowerLevel10k" "PowerLevel10k" "Oh-My-Posh" "Starship")
        else
            echo -e "${COLOR_BLUE}Choose prompt to install:${COLOR_RESET}"
            echo
    ...
    case $PROMPT_CHOICE in
            ...
            if $USE_GUM; then
                if gum_confirm "Install PowerLevel10k?"; then
                    ...
                    if gum_confirm "Install custom prompt?"; then
                        execute_command "curl -fLo \"$HOME/.p10k.zsh\" https://raw.githubusercontent.com/GiGiDKR/OhMyTermux/1.0.0/src/p10k.zsh" "Installing custom prompt" || error_msg "Failed to install custom prompt"
                        echo -e "\n# To customize prompt, run \`p10k configure\` or edit ~/.p10k.zsh." >> "$ZSHRC"
                        echo "[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh" >> "$ZSHRC"
                    else
                        echo -e "${COLOR_BLUE}You can customize prompt by running 'p10k configure'.${COLOR_RESET}"
                    fi
                fi
            else
                ...
                tput el
                if [[ "$CHOICE" =~ ^[yY]$ ]]; then
                    execute_command "git clone --depth=1 https://github.com/romkatv/powerlevel10k.git \"$HOME/.oh-my-zsh/custom/themes/powerlevel10k\" > /dev/null 2>&1 || true" "Installing PowerLevel10k"
                    sed -i 's/ZSH_THEME="robbyrussell"/ZSH_THEME="powerlevel10k\/powerlevel10k"/' "$ZSHRC"

                    printf "${COLOR_BLUE}Install custom prompt? (Y/n): ${COLOR_RESET}"
                    read -r -e -p "" -i "y" CHOICE
                    tput cuu1
                    tput el
                    if [[ "$CHOICE" =~ ^[yY]$ ]]; then
        ...
        "Oh-My-Posh")
            execute_command "pkg install -y oh-my-posh" "Installing Oh-My-Posh"

            # Optional Nerd font installation
            if [ ! -f "$HOME/.termux/font.ttf" ]; then
                execute_command "curl -fLo \"$HOME/.termux/font.ttf\" --create-dirs https://raw.githubusercontent.com/termux/termux-styling/master/app/src/main/assets/fonts/DejaVu-Sans-Mono.ttf" "Installing Nerd font"
            fi
            
            # Get complete list of themes
            ...
            if $USE_GUM; then
                ...
                echo
                # Calculate number of lines to clear (number of themes + 3 lines for additional text)
...
EOF
            ;;
        "Starship")
            execute_command "pkg install -y starship" "Installing Starship"
...
install_zsh_plugins() {
    ...
    subtitle_msg "❯ Plugin installation"

    if $USE_GUM; then
        mapfile -t PLUGINS_TO_INSTALL < <(gum_choose_multi "Select plugins to install (SPACE to select):" --height=8 --selected="All install" "zsh-autosuggestions" "zsh-syntax-highlighting" "zsh-completions" "you-should-use" "zsh-alias-finder" "All install")
...
install_font() {
    if $FONT_CHOICE; then
        ...
        if $USE_GUM; then
            ...
            echo
            printf "${COLOR_GOLD}Enter font number: ${COLOR_RESET}"
            tput setaf 3
            read -r -e -p "" -i "1" CHOICE
            tput sgr0
            tput cuu 15
        ...
        case $FONT in
            "Default font")
                execute_command "curl -fLo \"$HOME/.termux/font.ttf\" 'https://github.com/romkatv/powerlevel10k-media/raw/master/MesloLGS%20NF%20Regular.ttf'" "Installing default font"
        ...
        if ! $FULL_INSTALL; then
            if $USE_GUM; then
                if gum_confirm "Install XFCE?"; then
                    # Version selection
                    XFCE_VERSION=$(gum_choose "Select XFCE version to install:" --height=5 --selected="recommended" \
                    "minimal" \
                    "recommended" \
                    "customized")

                    # Browser selection (except for minimal version)
                    if [ "$XFCE_VERSION" != "minimal" ]; then
                        BROWSER_CHOICE=$(gum_choose "Select web browser:" --height=5 --selected="chromium" "chromium" "firefox" "none")
                    fi
                else
                    return
                fi
                ...
                read -r -e -p "" -i "y" CHOICE
                if [[ "$CHOICE" =~ ^[yY]$ ]]; then
                    echo -e "${COLOR_BLUE}Select XFCE version to install:${COLOR_RESET}"
                    echo
                    echo "1) Minimal"
                    echo "2) Recommended"
                    echo "3) Customized"
                    ...
                    if [ "$XFCE_VERSION" != "minimal" ]; then
                        echo -e "${COLOR_BLUE}Select web browser:${COLOR_RESET}"
                        echo
...
EOF
    ...
    execute_command "chmod +x start && mv start $PREFIX/bin" "Installing start script"
...
if pgrep -f 'apt|apt-get|dpkg|nala' > /dev/null; then
    zenity --info --text="Software is being installed in Termux or Proot. Please wait for these processes to finish before continuing."
    exit 1
fi

# Get process IDs of Termux-X11 and XFCE
...
# Stop Termux application only if PID exists
...
EOF
    ...
    execute_command "chmod +x $PREFIX/bin/kill_termux_x11" "Installing stop script"
...
EOF
    ...
    execute_command "chmod +x $PREFIX/share/applications/kill_termux_x11.desktop" "Creating shortcut"
}
...
install_proot() {
    if $PROOT_CHOICE; then
        ...
        execute_command "curl -O https://raw.githubusercontent.com/GiGiDKR/OhMyTermux/1.0.0/proot.sh" "Downloading PRoot script" || error_msg "Failed to download PRoot script"
        execute_command "chmod +x proot_dev.sh" "Executing PRoot script"
        
        # If credentials are already provided
        if [ -n "$PROOT_USERNAME" ] && [ -n "$PROOT_PASSWORD" ]; then
            ...
            if $USE_GUM; then
                if gum_confirm "Install Debian PRoot?"; then
                    execute_command "pkg install proot-distro -y" "Installing proot-distro"
...
install_utils() {
    ...
    if [ ! -f "$BASHRC_PROOT" ]; then
        error_msg ".bashrc file does not exist for user $USERNAME."
        execute_command "proot-distro login debian --shared-tmp --env DISPLAY=:1.0 -- touch \"$BASHRC_PROOT\"" "Configuring Debian Bash"
    fi
...
EOL
    ...
    if [ -f "$BASHRC" ]; then
        cat "$TMP_FILE" >> "$BASHRC"
        success_msg "✓ Termux Bash configuration"
    else
        touch "$BASHRC" 
        cat "$TMP_FILE" >> "$BASHRC"
        success_msg "✓ Creating and configuring Termux Bash"
    fi
    if [ -f "$ZSHRC" ]; then
        cat "$TMP_FILE" >> "$ZSHRC"
        success_msg "✓ Termux ZSH configuration"
    else
        touch "$ZSHRC"
        cat "$TMP_FILE" >> "$ZSHRC"
        success_msg "✓ Creating and configuring Termux ZSH"
    fi
    rm "$TMP_FILE"
}
...
if [ "$SHELL_CHOICE" = true ] || [ "$PACKAGES_CHOICE" = true ] || [ "$FONT_CHOICE" = true ] || [ "$XFCE_CHOICE" = true ] || [ "$PROOT_CHOICE" = true ] || [ "$X11_CHOICE" = true ]; then
    ...
    # Execute complete installation if no specific argument was provided
    if $EXECUTE_INITIAL_CONFIG; then
        initial_config
    fi
    install_shell
...
title_msg "❯ Saving installation scripts"
...
# Reload shell
if $USE_GUM; then
    if gum_confirm "Run OhMyTermux?"; then
        clear
